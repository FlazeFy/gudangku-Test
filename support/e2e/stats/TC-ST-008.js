...
describe('GudangKu E2E Test - TC-ST-008 - Stats', () => {
    const username = 'flazefy'
    const password = 'nopass123'
    const date = new Date().toISOString().replace(/:/g, '-')

    it('Pengguna Dapat Melihat Statistik Total Harga Inventory Berdasarkan Category', () => {
        ...
        cy.templateE2ELogin(username, password).then(() => {
            ...
            // Evidence - Step 1
            cy.screenshot(`TC-ST-008_Step-1-${date}`)    
            ...
            // Evidence - Step 2
            cy.screenshot(`TC-ST-008_Step-2-${date}`)   
            
            // Step 3: Pada section "Total Price Inventory By Category", Pengguna dapat melihat statistik Pie Chart dan tabel Context dan Total
            cy.get('#stats_total_inventory_by_category_holder').should('exist').prev('h2').should('have.text','Total Price Inventory By Category')
