...
describe('GudangKu E2E Test - TC-IN-007 - Inventory', () => {
    const username = 'flazefy'
    const password = 'nopass123'
    const date = new Date().toISOString().replace(/:/g, '-')
    let targetModal = ''

    it('Pengguna Dapat Menghapus Permanen Inventory Via Table List', () => {
        ...
        cy.templateE2ELogin(username, password).then(() => {
            ...
            // Evidence - Step 1
            cy.screenshot(`TC-IN-007_Step-1-${date}`)
            ...
            cy.get('#inventory_tb tbody .inventory-tr.deleted-inventory').first().then($el => {
                cy.wrap($el).find('.btn-delete').invoke('attr', 'data-bs-target').then(val => {
                    ...
                    // Step 4: Sistem akan menampilkan pesan "inventory permentally deleted"
                    cy.get('.swal2-popup', { timeout: 5000 }).should('exist').then(() => {
                        cy.get('.swal2-html-container').invoke('text').then((text)=>{
                            expect(text).to.equal('inventory permentally deleted')
                        })
                        // Evidence - Step 4
                        cy.screenshot(`TC-IN-007_Step-4-${date}`)
