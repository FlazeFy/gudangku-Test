...
describe('GudangKu E2E Test - TC-AL-005 - Analyze', () => {
    ...
    it('Pengguna Dapat Melihat Hasil Analisis Merk Inventory', () => {
        ...
        cy.templateE2ELogin(username, password).then(() => {
            ...
            // Evidence - Step 1
            cy.screenshot(`TC-AL-005_Step-1-${date}`)
            ...
            cy.screenshot(`TC-AL-005_Step-3-${date}`)
            cy.get('#edit_toolbar-section').contains('a', 'Analyze').click()
            cy.url().should('include','/analyze/inventory')

            // Step 4: Pada halaman analisis, pengguna dapat melihat hasil analisis merk berupa Total Merk Inventory, dan Average Merk Price
            cy.get('.swal2-popup:not(.swal2-loading)', { timeout: 10000 }).should('not.exist')
            cy.screenshot(`TC-AL-005_Step-4-${date}`)
            cy.get('#merk_analyze-section').within(() => {
                cy.contains('h3','The Merk')

                // Inventory Name, Inventory Merk
                const check_empty = ['.inventory_name','.inventory_merk']
                ...
                // Total Inventory
                cy.get('#total_inventory_merk').invoke('text').then((text) => {
                    const total = parseInt(text.trim())
                    expect(total).to.be.least(0)
                })
                // Average Price
                cy.get(`#inventory_price_avg_merk`).invoke('text').then((text) => {
