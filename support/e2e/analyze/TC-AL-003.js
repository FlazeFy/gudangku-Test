...
describe('GudangKu E2E Test - TC-AL-003 - Analyze', () => {
    ...
    it('Pengguna Dapat Melihat Hasil Analisis Unit & Volume Inventory', () => {
        ...
        cy.templateE2ELogin(username, password).then(() => {
            ...
            // Evidence - Step 1
            cy.screenshot(`TC-AL-003_Step-1-${date}`)
            ...
            cy.screenshot(`TC-AL-003_Step-3-${date}`)
            ...
            cy.screenshot(`TC-AL-003_Step-4-${date}`)
            cy.get('#unit_volume_analyze-section').within(() => {
                cy.contains('h3','The Unit & Volume')

                // Low Capacity Status
                cy.get('#low_capacity_holder').invoke('text').then((text) => {
                    const trimmed = text.trim()
                    expect(trimmed).to.satisfy(t => t === 'Normal Capacity' || t === 'At Low Capacity')
                })
                // Inventory Name, Inventory Category
                const check_empty = ['.inventory_name','.inventory_unit']
                ...
                // Total Inventory
                cy.get('#total_inventory_unit').invoke('text').then((text) => {
                    const total = parseInt(text.trim())
                    expect(total).to.be.least(0)
                })
                // Average Price
                cy.get(`#inventory_price_avg_unit`).invoke('text').then((text) => {
