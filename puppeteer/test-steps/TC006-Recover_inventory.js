...
puppeteer.launch({ headless: false }).then(async (browser) => {
    ...
    try {
        ...
        // Notes : Memilih item pertama yang sudah di delete
        await page.screenshot({
            path: `TC-006-Step 3 Result on ${date}.png`,
        })  
        const rows = await page.$$('#inventory_tb tbody tr.tr-item') // item holder

        for (let i = 0; i < rows.length; i++) {
            const firstDelete = await page.evaluate((row, rowIndex) => {
                const td_delete = row.querySelectorAll('td')[12] // edit column
                const td_name = row.querySelectorAll('td')[0] // inventory name column
        
                if (td_delete) {
                    // Test Step 4 : Pengguna menekan button yang berada pada kolom edit
                    const type_delete = td_delete.querySelector('input[name="is_editable"]')
                    const button = td_delete.querySelector('a.modal-btn')
                    button.click()
                    ...
                    return new Promise(resolve => {
                        setTimeout(() => {
                            ...
                            if (type_delete && type_delete.value == toogle) {
                                // Test Step 5 : Pada pop up validasi, pengguna menekan tombol Yes, Recover
        ...
        await page.waitForNavigation()
        await page.screenshot({
            path: `TC-006-Step 4 Result on ${date}.png`,
        })  
        ...
        let data = {
            ...
            tc_id: 'TC-006',
            tc_name: 'Recover inventory',
            tc_desc: 'Validate Test Steps for recover inventory',
            tc_data: {
                username: username,
                password: password,
                inventory: item
            },
            tc_evidence_name: [`TC-006-Step 3 Result on ${date}.png`,`TC-006-Step 4 Result on ${date}.png`],
            tested_at: date,
            tc_base_url: BASEURL
        };
        ...
        await page.evaluate(() => {
            alert(`TC-006 recover inventory Test Success`)
        }, [])
    } catch (error) {
